@echo off
echo ========================================
echo Charity-Izz Distributed Setup - hannah
echo ========================================
echo.

echo Step 1: Copying .env template...
copy env-templates\.env.hannah .env
echo.
...
echo Step 5: Starting Docker containers (MySQL Donation DB)...
...
echo Step 6: Running migrations on hannah database...
php artisan migrate --database=hannah --force
echo.

echo Step 7: Seeding database...
php artisan db:seed --database=hannah --force
echo.
...
echo ========================================
echo.
echo Your database is now running and exposed on port 3306
