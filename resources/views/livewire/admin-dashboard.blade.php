<div x-data="{ activeTab: '{{ $activeTab }}' }" x-init="$watch('activeTab', value => $wire.set('activeTab', value))">
    ...
    <!-- User Role Statistics (from Stored Procedure) -->
    @if(!empty($userRoleStats))
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h3 class="text-lg font-bold text-gray-900">User Role Statistics</h3>
                    <p class="text-xs text-gray-500">Data from stored procedure (sp_get_user_role_stats)</p>
                </div>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                @foreach($userRoleStats as $stat)
                    <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg p-4 border border-gray-200">
                        <div class="text-sm font-medium text-gray-600 capitalize mb-1">{{ $stat['role_name'] }}</div>
                        <div class="text-2xl font-bold text-gray-900">{{ number_format($stat['user_count']) }}</div>
                        @if($stat['latest_user_created'])
                            <div class="text-xs text-gray-500 mt-1">
                                Latest: {{ \Carbon\Carbon::parse($stat['latest_user_created'])->diffForHumans() }}
                            </div>
                        @endif
                    </div>
                @endforeach
