...
class DonationAllocationSeeder extends Seeder
{
    ...
    {
        // Get only approved recipients
        $approvedRecipients = Recipient::where('Status', 'Approved')->get();
        ...
        // Get campaigns that have collected some amount
        $campaigns = Campaign::where('Collected_Amount', '>', 0)->get();
    ...
    private function allocateCampaignFunds($campaign, $approvedRecipients)
    {
        ...
        foreach ($selectedRecipients as $index => $recipient) {
            ...
            if ($index === $recipientCount - 1) {
                $alreadyAllocated = DonationAllocation::where('Campaign_ID', $campaign->Campaign_ID)
            ...
            try {
                ...
                $existingAllocation = DonationAllocation::where('Recipient_ID', $recipient->Recipient_ID)
    ...
    private function generateAllocationDate($campaign)
    {
        ...
        else if ($now->greaterThan($campaignStart)) {
    ...
    {
        $totalAllocations = DonationAllocation::count();
        $totalAllocatedAmount = DonationAllocation::sum('Amount_Allocated');
        ...
        // Show per campaign
        $campaigns = Campaign::with('donationAllocations')->get();
        foreach ($campaigns as $campaign) {
            ...
            if ($allocated > 0) {
                ...
                $this->command->info("  Allocated: RM " . number_format($allocated, 2) . " / RM " . number_format($collected, 2) . " ({$percentage}%)");
                $this->command->info("  Recipients: {$campaign->donationAllocations->count()}");
            }
        }
        ...
        $recipients = Recipient::with('donationAllocations')->where('Status', 'Approved')->get();
        foreach ($recipients as $recipient) {
            $received = $recipient->donationAllocations->sum('Amount_Allocated');
            if ($received > 0) {
                $campaignCount = $recipient->donationAllocations->count();
